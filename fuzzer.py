...
class EvoGrammarFuzzer(Fuzzer):
    ...
    def generate_code(self, valid_variables):
        try:
            self.p.stdin.write(f"generate_code {json.dumps(valid_variables).replace(' ', '')}\n".encode("utf-8"))
            self.p.stdin.flush()
            code = ""
            while True:
                msg = self.p.stdout.readline().decode("utf-8").strip()
                if msg == "done":
                    ...
                    if msg.startswith('Warning:'): continue
                    # logging.info(f"[{self.id}]: sagem msg from domato process: {msg}")
                    code += msg + "\n"
            return code
        except BaseException as e:
            logging.info(f"[{self.id}]: fuzzer error: {e}")
            self.p.terminate()
            self.new_child()
            return self.generate_code(valid_variables)
