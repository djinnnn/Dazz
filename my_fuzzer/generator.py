...
def generate_html_elements(ctx, n):
    for i in range(n):
        ...
        ctx[
            'htmlvargen'] += '/* newvar{' + varname + ':' + tagtype + '} */ ' + varname + ' = document.createElement(\"' + tag + '\"); //' + tagtype + '\n'
...
def add_html_ids(matchobj, ctx, original_line=None):
    ...
    if tagname in _HTML_TYPES:
        ...
        ctx['htmlvargen'] += f"/* newvar{{ {varname}: {_HTML_TYPES[tagname]} }} */ " \
                             f" {varname} = document.getElementById(\"{varname}\"); " \
...
def generate_function_body(jsgrammar, htmlctx, num_lines,
    ...
    js = ''
    js += 'var fuzzervars = {};\n\n'
    if len(rdfuzz_context['variables']) == 0:
        js += "try{SetVariable(fuzzervars, window, 'Window');}catch(e){ }\ntry{SetVariable(fuzzervars, document, 'Document');}catch(e){ }\ntry{SetVariable(fuzzervars, document.body.firstChild, 'Element');}catch(e){ }\n\n"
    js += '//beginjs\n'
    js += htmlctx['htmlvargen']
    js_code, context = jsgrammar._generate_code(num_lines, tree_list, statement_map, htmlctx['htmlvars'], rdfuzz_context=rdfuzz_context)
    js += js_code
    js += '\n//endjs\n'
    js += 'var fuzzervars = {};\nfreememory()\n'

    rdfuzz_context['lastvar'] = context['lastvar']
    rdfuzz_context['variable_map'] = context['variable_map']

    variables = context['variables']
    for var_type in variables:
        # print(var_type, variables[var_type])
        if var_type not in rdfuzz_context['variables']:
            rdfuzz_context['variables'][var_type] = []
        for var_name in variables[var_type]:
            if var_name not in rdfuzz_context['variables'][var_type]:
                rdfuzz_context['variables'][var_type].append(var_name)
...
def generate_new_sample(template, htmlgrammar, cssgrammar, jsgrammar,
    ...
    while '<jsfuzzer>' in result:
        ...
        result = result.replace(
            '<jsfuzzer>',
            generate_function_body(jsgrammar, htmlctx, numlines, tree_list, statement_map, rdfuzz_context),
            1
        )

    return result, html, rdfuzz_context
...
class Generator:
    def __init__(self, selector: OneGramSelector, global_info: GlobalInfo):
        ...
        if browser_grammar == "CHROME":
            ...
            # self.jsgrammar.load_creators_for_vartype(os.path.join(self.grammar_dir, 'creators_for_vartype.txt'))
            self.jsgrammar.load_creators_for_symbol_and_vartype(os.path.join(self.grammar_dir, 'creators_for_symbol_and_vartype.txt'))
        ...
        self.html = ""
        # self.css = ""
        self.rdfuzz_context = {'lastvar':0, 'variables':{}, 'variable_map': {}}
    ...
    def generate_one(self) -> str:
        ...
        # symbols = []
        # for symbol in self.jsgrammar._creators:
        #     if len(self.jsgrammar._creators[symbol]) < 10: continue
        #     symbols.append(symbol)
        # print(len(symbols))

        # print('line' in symbols)
        ...
        #         # print(symbol, var_type)
        #         if symbol not in self.jsgrammar.creators_for_symbol_and_vartype:
        #             self.jsgrammar.creators_for_symbol_and_vartype[symbol] = {}
                
        #         # filtered_creators = list(filter(lambda x: var_type in str(x['original_line'].split('=')[-1]), self.jsgrammar._creators[symbol]))
        #         filtered_creators = list(filter(lambda x: "<" + var_type + ">" in str(x['original_line']), self.jsgrammar._creators[symbol]))
        #         if var_type not in self.jsgrammar.creators_for_symbol_and_vartype[symbol] and len(filtered_creators) != 0:
        #             self.jsgrammar.creators_for_symbol_and_vartype[symbol][var_type] = filtered_creators
            
        # print(len(self.jsgrammar.creators_for_symbol_and_vartype))
        # with open('temp2.txt', 'w') as file:
        #     json.dump(self.jsgrammar.creators_for_symbol_and_vartype, file)
        # shutil.move('temp2.txt', self.jsgrammar.creators_for_symbol_and_vartype_file)
        ...
        self.rdfuzz_context = {'lastvar':0, 'variables':{}, 'variable_map':{}}
        result, self.html, self.rdfuzz_context = generate_new_sample(self.template, self.htmlgrammar, self.cssgrammar,
                                     self.jsgrammar, self.derivation_tree_list, self.statement_to_tree)

        return result


    def generate_code(self, valid_variables):

        total_count = 0
        alive_count = 0
        deleted_keys = []
        
        # Dazz: dynamic context harvesting
        for var_type in self.rdfuzz_context['variables']:
            total_count += len(self.rdfuzz_context['variables'][var_type])
            self.rdfuzz_context['variables'][var_type] = list(filter(lambda x: x in valid_variables, self.rdfuzz_context['variables'][var_type]))
            alive_count += len(self.rdfuzz_context['variables'][var_type])
            if len(self.rdfuzz_context['variables'][var_type]) == 0:
                deleted_keys.append(var_type)
        
        for var_type in deleted_keys:
            del self.rdfuzz_context['variables'][var_type]
        ...
        code = generate_function_body(self.jsgrammar, htmlctx, _N_CODE_LINES, self.derivation_tree_list, self.statement_to_tree, self.rdfuzz_context)
...
def main():
    ...
    while True:
        ...
        if msg == "generate":
            ...
            print("stored")
            sys.stdout.flush()
        elif msg.startswith("generate_code"):
            valid_variables = json.loads(msg.split(" ")[1])
            # print("generate code with valid variables: ", valid_variables)
            code = generator.generate_code(valid_variables)
            print(code)
            print("done")
            sys.stdout.flush()
