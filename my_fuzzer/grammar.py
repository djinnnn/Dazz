...
def randomly_combine_adjacent(arr):
    """
    随机合并数组中相邻的两个字符串，生成一个新的数组。
    处理过程：
      - 遍历原数组
      - 如果当前元素不是最后一个元素，则随机决定是否和后一个元素合并
      - 如果合并，则将这两个字符串连接起来加入新数组，并跳过下一个元素
      - 否则，直接将当前元素加入新数组
    """
    result = []
    i = 0
    while i < len(arr):
        # 检查是否存在后续元素
        if i < len(arr) - 1 and random.choice([True, False]):
            # 随机选择合并当前元素和后一个元素
...
def escape_template_string(input_str):
    escaped_string = re.sub(r'([`\\"])', r'\\\1', input_str)  # 转义 `, \, "
...
class Grammar(object):
    ...
    def __init__(self, selector: OneGramSelector, global_info: GlobalInfo, grammar_type,
        ...
        self._sagem_rule_select_prob = 0.75
        self._sagem_rule_select_prob = 1
        self._whilestate_prob = 0.3
        ...
        # self.creators_for_vartype = {}
        # self.creators_for_vartype_file = None

        self.creators_for_symbol_and_vartype = {}
        self.creators_for_symbol_and_vartype_file = None
    ...
    def _generate_code(self, num_lines,
                       ...
                       rdfuzz_context={'lastvar':0,'variables':{}, 'variable_map':{}}):
        """Generates a given number of lines of code."""

        # print(f"_generate_code with rdfuzz_context: {rdfuzz_context}", file=sys.stderr)
        context = {
            'lastvar': rdfuzz_context['lastvar'],
            'lines': [],
            'variables': {},
            'valid_variables': rdfuzz_context['variables'],
            'variable_map': rdfuzz_context['variable_map'],
            'interesting_lines': [],
        ...
        while len(context['lines']) < num_lines:
            ...
            creator = None
            try:
                # Dazz: type-driven API association
                if (random.random() < self._sagem_rule_select_prob) and (
                    len(tmp_context['valid_variables']) > 0):
                    var_types = [x for x in list(tmp_context['valid_variables'].keys()) if x not in _DAZZ_NONINTERESTING_TYPES]
                    var_type = random.choice(var_types)
                    # print(f'Dazz++: type count: {len(var_types)}', file=sys.stderr)

                    if var_type in self.creators_for_symbol_and_vartype['line']:
                        creators = self.creators_for_symbol_and_vartype['line'][var_type]
                        # print(f'Dazz++: type-related API count: {len(creators)} ({var_type})', file=sys.stderr)
                        if len(creators) > 0:
                ...
                context = tmp_context
                # print(len(context['lines']), file=sys.stderr)
        ...
        # if self.is_js:
        #     for i in range(0, 5):
        #         context['lines'] = randomly_combine_adjacent(context['lines'])
        #     context['lines'] = map(lambda line: 'while (True) {<line>; if (sagem_random() > 0.5) break;}'.replace('<line>', line) \
        #         if random.random() < self._whilestate_prob else \
        #         line, context['lines'])
        ...
        if self.is_js:
            ...
            for line in guarded_lines:
                new_line = f"try {{ {line};  UpdateFeedback(String.raw`{escape_template_string(line)}`, \"Valid\"); }} catch(e) {{  UpdateFeedback(String.raw`{escape_template_string(line)}`, e.name) }}"
                new_guarded_lines.append(new_line)
            guarded_lines = new_guarded_lines
        return '\n'.join(guarded_lines), context
    ...
    def _select_creator(self, symbol, recursion_depth, force_nonrecursive,
        ...
        if not cdf:
            if self.grammar_type != "js":
                ...
                # Dazz: type-driven parameter generation
                if (random.random() < self._sagem_rule_select_prob and len(creators) >= 10):
                    
                    filtered_creators = []
                    for var_type in list(context['valid_variables'].keys()):
                        if symbol in self.creators_for_symbol_and_vartype:
                            if var_type in self.creators_for_symbol_and_vartype[symbol]:
                                filtered_creators = self.creators_for_symbol_and_vartype[symbol][var_type] + filtered_creators
    ...
    def _generate(self, symbol, context,
        ...
        # Dazz: state-aware object propagation
        # Check if we already have a variable of the given type.
        if ((symbol in context['variables'] or symbol in context['valid_variables'])):
            if (symbol in context['valid_variables'] or
                    len(context['variables'][symbol]) > self._max_vars_of_same_type):

                variables = []
                if symbol in context['valid_variables']:
                    variables = context['valid_variables'][symbol]
                elif symbol in context['variables'] and len(context['variables'][symbol]) > 0:
                    variables = context['variables'][symbol]
                variable = variables[random.randint(0, len(variables) - 1)]
        
        # if ((symbol in context['variables'])):
        #     if (len(context['variables'][symbol]) > self._max_vars_of_same_type):

        #         variables = []
        #         if symbol in context['variables'] and len(context['variables'][symbol]) > 0:
        #             variables = context['variables'][symbol]
        #         variable = variables[random.randint(0, len(variables) - 1)]
        ...
        # TODO: filter the creators set for selection using object reuse strategy.
    ...
    def _expand_rule(self, symbol, rule, context,
        ...
        for part in rule['parts']:
            ...
            if part['type'] == 'text':
                ...
                expanded = '/* newvar{' + var_name + ':' + var_type + '} */ ' + var_name
    ...
    def generate_root(self):
        ...
        if self._root:
            ...
            # context['force_var_reuse'] = True if len(context['valid_variables']) > 0 else False
...
``
    ...
    # def load_creators_for_vartype(self, path):
    #     self.creators_for_vartype_file = path
    #     print(self.creators_for_vartype_file)
    #     with open(self.creators_for_vartype_file) as file:
    #         self.creators_for_vartype = json.load(file)

    def load_creators_for_symbol_and_vartype(self, path):
        self.creators_for_symbol_and_vartype_file = path
        print(self.creators_for_symbol_and_vartype_file)
        with open(self.creators_for_symbol_and_vartype_file) as file:
            self.creators_for_symbol_and_vartype = json.load(file)
       
        # print(self.creators_for_symbol_and_vartype['line'])
        # count = 0
        # for var_type in self.creators_for_symbol_and_vartype['line']:
        #     count += len(self.creators_for_symbol_and_vartype['line'][var_type])
